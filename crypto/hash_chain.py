...
class MicroLedger:
    ...
    def verify_local_integrity(self) -> bool:
        """Verifies the internal chain history (from genesis or earliest retained entry)."""
        if not self._history:
            return True
        first_entry = self._history[0]
        if first_entry.seq == 1 and first_entry.prev_digest != self.GENESIS_HASH:
            return False
        current_prev = first_entry.prev_digest
        expected_seq = first_entry.seq
